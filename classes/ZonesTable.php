...
class ZonesTable extends \WP_List_Table {
    ...
    function __construct(){
        ...
        parent::__construct( array(
            'singular'  => 'zone',     //singular name of the listed records
            'plural'    => 'zones',    //plural name of the listed records
            'ajax'      => false        //does this table support ajax?
        ) );
        
        //!!!! Better way to inject these?
        $this->zone_obj = new Zones();
    ...
    */
    function column_default($item, $column_name){
        switch($column_name){
            ...
            case 'ads':
                $relations = $this->banner_obj->getRelationsByZone($item['ID']);
                if(!empty($relations)){
                    $output = '';
                    foreach($relations as $banner){
                        $banner = new Banner($banner['banner_id']);
                        $output .= $banner->showPreview();
                    }
                    return $output;
                }
                return 'No ads attached';
            break;
            default:
    ...
    */
    function column_title($item){
        ...
        $actions = array(
            ...
            'stats'     => '<a href="'.sprintf('?page=%s&subpage=Overview&id=%s',$_REQUEST['page'], $item['ID']).'">Stats</a>'
        );
        
        $actions = apply_filters('bannerlid_zoneactions', $actions, $item);
    ...
    function process_bulk_action() {
        ...
        if( 'delete'===$this->current_action() ) {
            if(!empty($_REQUEST['zone'])){
                foreach($_REQUEST['zone'] as $id){
                    $this->zone_obj->delete($id);
